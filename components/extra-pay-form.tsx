...
type ExtraPayFormProps = {
  ...
  /** When false, start expanded (e.g. dual-sheet compare). Default collapsed. */
  defaultOpen?: boolean;
};
...
export function ExtraPayForm({
  ...
  const otherPayTotal = regularPay + vacationPay + bonusTotal;
  ...
  return (
    <section className="summary-card extra-pay-card no-print">
      <button
        type="button"
        className="extra-pay-toggle"
        aria-expanded={isOpen}
        aria-controls={panelId}
        onClick={() => setIsOpen((open) => !open)}
      >
        <span className="extra-pay-toggle-copy">
          <span className="extra-pay-toggle-title">Other pay</span>
          <span className="extra-pay-toggle-badge">{formatMoney(otherPayTotal)}</span>
        </span>
        <ChevronDown
          aria-hidden="true"
          className={`extra-pay-chevron${isOpen ? " is-open" : ""}`}
        />
      </button>

      <div
        id={panelId}
        className={`extra-pay-collapse${isOpen ? " is-open" : ""}`}
        aria-hidden={!isOpen}
      >
        <div className="extra-pay-collapse-inner">
          ...
          <div className="extra-pay-actions">
            {readOnly ? null : (
              <Button className="no-print" variant="outline" tabIndex={isOpen ? undefined : -1} onClick={onAddBonus}>
                <Plus data-icon="inline-start" />
                Add bonus
              </Button>
            )}
            <p className="extra-pay-total">
              Regular + vacation + bonuses {formatMoney(otherPayTotal)}
            </p>
          </div>
        </div>
      </div>
