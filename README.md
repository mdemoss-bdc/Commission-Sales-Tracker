...
After changing this schema, re-run the full `supabase/schema.sql` in the SQL editor (safe to re-run). That adds `previous_data`, `pending_admin_approval`, and the review RPCs. Add `https://your-domain/reset-password` to the Supabase Auth redirect URLs.
...
2. The rep sees **Manager Updates Waiting for Review**: accept or decline brand-new deals, and for matching stock numbers pick **Keep Mine** or **Use Manager’s**. **Confirm & Submit to Manager** sets `status = pending_manager_approval` and stores the manager’s original push in `previous_data`. Live records stay frozen.
