...
1. **Push to employee** never overwrites live data. The click packages the full current worksheet (`deals`, `vacation_hours`, `hourly_rate`, `vacation_pay`, `bonuses`) into `push_drafts_to_employee`. Pushed rows are flagged `awaiting_review` (or `pending_rep_review` if that enum is all that exists). A notification is inserted for that rep only. If that push is still waiting, **Cancel / Delete Push** recalls it back to an editable draft (`recall_pending_push`) so you can edit and send it again.
