...
Discard a failed query. Open a **new** SQL query (do not re-run the old editor tab). Scroll to the bottom and delete any auto-appended `ALTER TABLE rec` line before running. Composite variables are named `found_row` so the editor does not treat `rec` as a table. `CREATE OR REPLACE` cannot change a function’s return type; the schema drops existing functions first so a re-run is not blocked by `42P13`.
