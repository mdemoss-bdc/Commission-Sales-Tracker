...
2. The rep opens the pushed worksheet. **Your Current Worksheet** (live) sits on top of **Manager / Admin Pushed Worksheet**. Differing cells and extra/missing rows are highlighted. The employee edits the bottom table in place (or uses **Copy my value**) and hits **Confirm Changes & Push Back to Manager**. That submits the corrected bottom table as `pending_manager_approval`. Live records stay frozen until the manager finalizes. There is no radio-button popup.
