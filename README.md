...
2. The rep’s month view (`/m/[monthId]`) docks a persistent amber banner when there is an unread `pay_push` / `pay_sheet` notification or the sheet status is `awaiting_review` / `pushed`: **Awaiting Employee Review: Your manager has pushed an updated pay sheet.** Notifications stay unread until the rep clicks **Review Pushed Numbers & Sync** or **Dismiss**. Review opens the stacked comparison of their logged draft vs the manager buffer (units, trades, gross, commission). **Accept & Lock** writes the manager buffer onto their active sheet and marks the notification read. **Flag Dispute / Leave Note** keeps the live worksheet and sends a `rep_notes` comment back. Confirming edits still submits the complete sheet as `pending_manager_approval`. Live records stay frozen until the manager finalizes, except after **Accept & Lock**.
