...
export function isMissingRelation(message: string, code?: string): boolean {
  return (
    ...
    message.includes("commit_proposed_to_live") ||
...
const PAY_TRACKER_STATE_SELECTS = [
  PAY_TRACKER_STATE_SELECT,
  "id,user_id,employee_id,month_id,status,state,admin_pushed_snapshot,rep_draft,approval_diffs,pay_delta,finalized_label,location_id,created_by,updated_at",
...
async function updatePayTrackerChain(
  employeeId: string,
  ...
  if (!supabase) return "Not signed in.";
  const now = new Date().toISOString();
  let current: Record<string, unknown> = { ...patch, updated_at: now };
  for (let attempt = 0; attempt < 6; attempt++) {
    const { error } = await supabase.from(PAY_TRACKER_STATE_TABLE).update(current).eq("id", employeeId);
    if (!error) return null;
    ...
    const column = missingColumnName(error.message);
    if (!column || !(column in current)) return error.message;
    const next = { ...current };
    delete next[column];
    current = next;
  }
...
export async function acceptPushNoChanges(userId?: string): Promise<string | null> {
...
export async function lockAcceptedPushToLive(
  ...
  const accepted = realDecisions.filter((decision) => decision.action !== "decline");
  const declined = realDecisions.filter((decision) => decision.action === "decline");
  if (accepted.length > 0) {
    const lockError = await commitAcceptedDecisionsToLive(accepted);
    if (lockError) return lockError;
  }
  if (declined.length > 0) {
    const declineError = await applyReviewResolutions(declined);
    if (declineError) return declineError;
  }
  ...
  const userId = await currentUserId();
  if (userId && (accepted.length > 0 || declined.length > 0)) {
    const keepIds = submittedKeepIds(accepted);
    const sweepError = await sweepRemainingEmployeeReview(userId, keepIds);
...
async function recallPayTrackerState(repId: string): Promise<string | null> {
  ...
  if (isMissingColumn(error.message, error.code)) {
    ...
    if (isMissingRelation(retryError.message, retryError.code) || isMissingColumn(retryError.message, retryError.code)) {
      return null;
    }
...
async function markRepRosterUnready(userId: string): Promise<string | null> {
  const supabase = getSupabase();
  if (!supabase) return "Not signed in.";
  const { error } = await supabase.from(USER_PROFILES_TABLE).update({ roster_ready: false }).eq("id", userId);
  if (!error) return null;
...
> {
  ...
  for (const columns of selects) {
    ...
    if (!error) {
      rows = ((data ?? []) as unknown as DealRow[]).map(normalizeDealRow);
      break;
    }
    lastError = error;
...
async function promoteDraftsToEmployeeReview(ids: string[]): Promise<string | null> {
  ...
  for (const id of ids) {
    ...
    const error = await updateDealRow(id, {
      ...
      proposed_data: isPayload(rec.staged_data) ? rec.staged_data : asJsonObject(rec.proposed_data),
      reject_reason: null,
      updated_at: now,
    });
    if (error) {
...
export async function recallPendingPush(repId: string): Promise<string | null> {
  ...
  if (loaded.error) return loaded.error;
  for (const row of loaded.rows) {
    if (isSyntheticPayTrackerDealId(row.id)) continue;
    if (!isInFlightEmployeePush(row.status) && !isAwaitingRepReview(row.status)) continue;
    const updateError = await updateDealRow(row.id, {
      status: "draft",
      proposed_data: {},
      staged_data: isPayload(row.staged_data) ? row.staged_data : {},
      reject_reason: null,
      updated_at: now,
    });
...
async function writeDealRecord(
  run: (
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    const next = { ...current };
    let stripped = false;
    for (const name of OPTIONAL_DEAL_COLUMNS) {
      if (name in next) {
        delete next[name];
        stripped = true;
      }
    }
    if (!stripped) return error.message;
    current = next;
  }
  return "Could not save deal.";
}
...
async function loadDealRow(id: string): Promise<{ row: DealRow | null; error: string | null }> {
  ...
  for (const columns of selects) {
    ...
    if (!error) {
      const row = (data as unknown as DealRow | null) ?? null;
      return { row: row ? normalizeDealRow(row) : null, error: null };
    }
...
async function applyReviewResolutions(decisions: ReviewResolution[]): Promise<string | null> {
  ...
  for (const decision of decisions) {
    ...
    const resolved = decision.live_data ?? managerPushPayload(rec) ?? rec.staged_data;
    ...
    const prior = decision.action === "accept" ? empty : managerPushPayload(rec) ?? rec.staged_data ?? empty;
    const approvalPatch = {
...
async function commitAcceptedDecisionsToLive(decisions: ReviewResolution[]): Promise<string | null> {
  const supabase = getSupabase();
  if (!supabase) return "Not signed in.";
  const empty = {};
  const now = new Date().toISOString();
  for (const decision of decisions) {
    ...
    const rec = loaded.row;
    if (!rec) continue;
    const payload = isPayload(decision.live_data) ? decision.live_data : commitLivePayload(rec);
    const targetId = decision.live_id && decision.live_id !== rec.id ? decision.live_id : rec.id;
    const error = await updateDealRow(targetId, {
      live_data: payload,
      staged_data: empty,
      proposed_data: empty,
      previous_data: empty,
...
function isLockableDealStatus(status: string | null | undefined): boolean {
  return (
    status === "pending_manager_approval" ||
...
async function lockDealIdsLive(ids: string[]): Promise<string | null> {
  ...
  for (const id of realIds) {
    ...
    const rec = loaded.row;
    if (!rec) continue;
    if (!isLockableDealStatus(rec.status)) continue;
    const live = commitLivePayload(rec);
    const error = await updateDealRow(id, {
      live_data: live,
      staged_data: empty,
      proposed_data: empty,
