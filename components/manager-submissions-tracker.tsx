...
export function ManagerSubmissionsTracker({ rows }: { rows: ManagerStoreStatus[] }) {
  ...
  return (
    <section className="summary-card no-print manager-submissions">
      <button
        type="button"
        className="manager-submissions-toggle"
        aria-expanded={open}
        onClick={() => setOpen((current) => !current)}
      >
        <span>
          Manager Submissions ({completeCount} Complete, {pendingCount} Pending)
        </span>
