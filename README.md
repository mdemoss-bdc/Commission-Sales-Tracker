...
Discard a failed query. Do not run any dashboard-injected `ALTER TABLE rec ENABLE ROW LEVEL SECURITY` — `rec` is a PL/pgSQL variable, not a table. `CREATE OR REPLACE` cannot change a function’s return type; the schema drops existing functions first so a re-run is not blocked by `42P13`.
