...
**Employee roster:** Admin and Manager screens list sales reps A–Z by full name (managers see only their store; admins must pick a store first). Selecting an employee on the Admin dashboard opens **Admin Master Sheet: [Employee Name]** — an isolated workbench with the standard deals table, bonus controls, and vacation pay. Admin edits write only to `admin_employee_sheets.sheet_data`. Reps never read that table; they only see `deal_records` / `pay_tracker_state`. The two ledgers never overwrite each other in real time. **Push Sheet to Employee & Manager** marks the admin row `status = 'pushed'`, copies a comparison snapshot into `admin_pushed_snapshot` / `proposed_data`, and notifies the rep. It does **not** erase deals the sales rep already entered locally. After you push, the admin roster shows **Pending Employee & Manager Approval**. The store manager (for example Huntington) sees the same pushed sheet with **Pending Employee Acceptance** and cannot authorize yet. When the employee accepts with no edits, the manager badge is green **Sales Rep Authorized (No Changes Made)** — **Authorize & Push to Admin** locks Admin’s sheet unchanged as `admin_final_approved`. When they edit, the amber badge is **Employee Submitted Changes (+$XX.XX / -$XX.XX diff)**; clicking it opens an itemized diff of added, deleted, or edited deals, spiffs, and bonuses. Managers then **Authorize & Push to Admin** (overwrites `admin_employee_sheets.sheet_data` and finalizes the sheet) or **Reject Changes** with a reason (`rejected_by_manager`) so the employee can fix and re-submit. Admin never sees those manager buttons. **Delete / Reset Push** wipes `proposed_data`, pending payloads, unread push notifications, and resets the employee sheet to draft without clearing the admin master `sheet_data`. When a manager authorizes, the admin roster shows **Finalized on Admin dashboard**.
...
1. **Push Sheet to Employee & Manager** never overwrites live data, the admin master `sheet_data`, or the sales rep’s working `pay_tracker_state.state`. The click packages the full current Admin Master Sheet (`deals`, `gross`, `units`, `trades`, F&I, `bonuses`, vacation, `month_id`, `employee_id`), writes that snapshot to `admin_pushed_snapshot` / `proposed_data` as `admin_pushed`, and sets `admin_employee_sheets.status = 'pushed'`. A notification is inserted for that rep only. The sales-rep dashboard and month view fetch their own working sheet plus the comparison snapshot. **Check for Updates** next to **Add sales sheet** force-refetches if realtime is delayed. **Delete / Reset Push** (`recall_pending_push`) clears `proposed_data`, the approval-chain snapshot/draft/diffs, unread `pay_push` / `pay_sheet` notifications, sets the employee back to draft, and returns the admin master status to `draft` without wiping `sheet_data`.
