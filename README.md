...
1. **Push to employee** never overwrites live data. The click packages the full current worksheet (`deals`, `vacation_hours`, `hourly_rate`, `vacation_pay`, `bonuses`) into `push_drafts_to_employee`. Pushed rows are flagged `awaiting_review` (or `pending_rep_review` / `pushed` if that enum is all that exists). A notification is inserted for that rep only. The sales-rep dashboard and month view fetch `deal_records` for `auth.uid()` (and the open `monthId`) from Supabase and override stale localStorage when a pushed sheet is waiting. **Check for Updates** next to **Add sales sheet** force-refetches if realtime is delayed. If that push is still waiting, **Cancel / Delete Push** recalls it back to an editable draft (`recall_pending_push`) so you can edit and send it again.
