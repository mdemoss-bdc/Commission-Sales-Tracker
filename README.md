...
**Employee roster:** Admin and Manager screens list sales reps A–Z by full name (managers see only their store; admins must pick a store first). Open a name to enter that rep’s staging sheet. Green (**Ready / Submitted**) means the sheet is `pending_manager_approval` (or every deal is already submitted / you used **Authorize / Skip for Rep**). Amber (**Awaiting Employee Review**) is `pending_rep_review` or `awaiting_review`. After an admin or manager **Push to employee**, the click upserts the staging payload (`deals`, gross, units, trades, F&I, bonuses, vacation, `month_id`, `employee_id`) into `pay_tracker_state` for that employee, creates/promotes `deal_records`, and writes a `user_notifications` row for that employee’s `user_id` (`notify_rep_on_sheet_push`). The header bell shows a badge, and the sales rep dashboard and month view (`/m/[monthId]`) fetch `pay_tracker_state` for `auth.uid()`. If that row’s status is `awaiting_review`, they show **Manager has pushed an updated pay sheet for your review.** with **Review Pushed Sheet** (stacked live vs manager comparison), **Accept & Lock**, and **Flag Dispute / Leave Note**, and the on-screen numbers match the pushed sheet. **Check for Updates** next to **Add sales sheet** force-refetches if realtime is delayed. The assigned store manager’s roster updates to Awaiting Employee Review so they can track progress or skip with **Authorize / Skip for Rep**. **Push All to Admin** stays disabled until every visible rep is green, then locks that store’s ready sheets into live `active` records.
...
1. **Push to employee** never overwrites live data. The click packages the full current worksheet (`deals`, `gross`, `units`, `trades`, F&I, `bonuses`, vacation, `month_id`, `employee_id`) and upserts it into `pay_tracker_state` via `upsert_pay_tracker_state` (and `push_drafts_to_employee`). Pushed rows are flagged `awaiting_review` (or `pending_rep_review` / `pushed` if that enum is all that exists). A notification is inserted for that rep only. The sales-rep dashboard and month view fetch `pay_tracker_state` (and `deal_records`) for `auth.uid()` from Supabase and override stale localStorage when a pushed sheet is waiting. **Check for Updates** next to **Add sales sheet** force-refetches if realtime is delayed. If that push is still waiting, **Cancel / Delete Push** recalls it back to an editable draft (`recall_pending_push`) so you can edit and send it again.
2. The rep sees the pending-review banner on `/` and `/m/[monthId]`. **Review Pushed Sheet** opens the stacked comparison: **Your Current Worksheet** on top of **Manager / Admin Pushed Worksheet**. Differing vacation hours, hourly rate, vacation total, and bonuses are highlighted. The employee types directly in the bottom deals table and the bottom Other pay section (vacation hours, hourly rate, bonus names/amounts, add or delete bonuses) and hits **Confirm Changes & Push Back to Manager**, or uses **Accept & Lock** to take the manager values as-is. **Flag Dispute / Leave Note** keeps the live worksheet and sends a `rep_notes` comment back. That submits the complete sheet as `pending_manager_approval`. Live records stay frozen until the manager finalizes.
